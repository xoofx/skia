...
/**
    ...
    This call must be balanced with a call to sk_data_unref().
*/
SK_API_C sk_data_t* sk_data_new_from_malloc(const void* memory, size_t length);
/**
...
*/
SK_API_C sk_data_t* sk_data_new_subset(const sk_data_t* src, size_t offset, size_t length);
/**
    Increment the reference count on the given sk_data_t. Must be
    balanced by a call to sk_data_unref().
*/
SK_API_C void sk_data_ref(const sk_data_t*);
/**
...
*/
SK_API_C void sk_data_unref(const sk_data_t*);
/**
    Returns the number of bytes stored.
*/
SK_API_C size_t sk_data_get_size(const sk_data_t*);
/**
    Returns the pointer to the data.
 */
SK_API_C const void* sk_data_get_data(const sk_data_t*);
/**
    Create a new dataref the file with the specified path.
    If the file cannot be opened, this returns NULL.
*/
SK_API_C sk_data_t* sk_data_new_from_file(const char* path);
/**
    Attempt to read size bytes into a SkData. If the read succeeds, return the data,
    else return NULL. Either way the stream's cursor may have been changed as a result
    of calling read().
*/
SK_API_C sk_data_t* sk_data_new_from_stream(sk_stream_t* stream, size_t length);
/**
...
*/
SK_API_C const uint8_t* sk_data_get_bytes(const sk_data_t*);
