...
*/
SK_API_C sk_string_t* sk_string_new_empty();
/**
...
*/
SK_API_C sk_string_t* sk_string_new_with_copy(const char* src, size_t length);

/**
    Deletes the string.
*/
SK_API_C void sk_string_destructor(const sk_string_t*);

/**
    Returns the number of bytes stored in the UTF 8 string. Note that this is the number of bytes, not characters.
*/
SK_API_C size_t sk_string_get_size(const sk_string_t*);
/**
    Returns the pointer to the string.
 */
SK_API_C const char* sk_string_get_c_str(const sk_string_t*);
